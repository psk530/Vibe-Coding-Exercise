...
4. 채팅창은 마크다운을 렌더링하지 않고 입력한 문자 그대로 화면에 보여줍니다. 그러므로 #, *, **, -, |, `, > 같은 기호나 이모지를 절대 쓰지 마세요.
   강조하고 싶은 부분도 별표 없이 그냥 평문으로 쓰세요 (틀린 예: "**트래픽**이 40% 감소" / 올바른 예: "트래픽이 40% 감소").
   목록이 필요하면 하이픈이나 글머리 기호 대신 "첫째, ~. 둘째, ~." 또는 줄바꿈된 문장으로 풀어서 쓰세요.
...
def chat(req: ChatRequest):
    ...
    try:
        ...
        try:
            ...
            print(f"chat error: online_call_error={e1!r} base_call_error={e2!r}")
